// The committed migration SQL, inlined as a JS string so it ships inside the
// bundle without any Metro asset resolution. Each entry's `name` matches the
// migration directory, so applyMigrations records it in _migrations and never
// re-runs it.
//
// When the schema evolves: run `prisma migrate dev` on your laptop, commit the
// new prisma/migrations/<ts>_<name>/migration.sql, copy it to
// prisma/migrations/_bundled/ and append an entry here.
