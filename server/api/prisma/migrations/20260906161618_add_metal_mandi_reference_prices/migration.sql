...
-- NOTE: prisma migrate also emits
--   CREATE INDEX "recycler_authorization_status_idx" ON "recycler"("authorization_status");
-- here, because schema.prisma carries @@index([authorizationStatus]). That index
-- was deliberately dropped in 20260831211646_constraints_views_indexes and replaced
-- by the partial index `recycler_valid` (WHERE authorization_status = 'VALID'), which
-- is what DB.md section 4 specifies. It is omitted on purpose — re-adding it would
-- undo that choice. Prisma will keep re-emitting it as drift on future migrations;
-- keep omitting it.
